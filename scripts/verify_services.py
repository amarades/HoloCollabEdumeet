...
def check_service(service):
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"{Fore.GREEN}[OK]{Style.RESET_ALL} {service['name']}: {Fore.GREEN}Running{Style.RESET_ALL}")
            return True
        print(
            f"{Fore.RED}[FAIL]{Style.RESET_ALL} {service['name']}: "
            f"{Fore.RED}Error (Status {response.status_code}){Style.RESET_ALL}"
        )
        return False
    except requests.exceptions.ConnectionError:
        print(f"{Fore.RED}[FAIL]{Style.RESET_ALL} {service['name']}: {Fore.RED}Not running{Style.RESET_ALL}")
        return False
    except requests.exceptions.Timeout:
        print(f"{Fore.YELLOW}[WARN]{Style.RESET_ALL} {service['name']}: {Fore.YELLOW}Timeout{Style.RESET_ALL}")
        return False
    except Exception as e:
        print(f"{Fore.RED}[FAIL]{Style.RESET_ALL} {service['name']}: {Fore.RED}Error - {str(e)}{Style.RESET_ALL}")
        return False
